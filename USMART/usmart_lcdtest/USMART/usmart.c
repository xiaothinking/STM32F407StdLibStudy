...
void usmart_scan(void)
{
	...
	if((g_uart1_receive_finish == 1) && (g_uart1_tick == 0))//串口接收完成？
	{			
      g_uart1_receive_finish = 0;		
		...
		len=m_length;	//得到此次接收到的数据长度
		ar_uart1_protocol[len]='\0';	//在末尾加入结束符. 
		sta=usmart_dev.cmd_rec(ar_uart1_protocol);//得到函数各个信息
		if(sta==0)usmart_dev.exe();	//执行函数 
		else 
		{  
			len=usmart_sys_cmd_exe(ar_uart1_protocol);
